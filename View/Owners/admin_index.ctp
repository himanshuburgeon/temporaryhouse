...
<div>
	<article>
		<header>
			<h2><?php echo __('owner_manager') ?></h2>
			<div style="float:right;">
				...
				<?php echo $this->Html->link(__('new'), array('controller'=>'owners', 'action' => 'admin_add'), array('escape' => false,'class'=>'button'));?>
			</div>
		</header>
	</article>
	...
	<?php echo $this->Form->create('Owner', array('name'=>'owner','action' => 'delete/','id'=>'OwnerDeleteForm','onSubmit'=>'return validate(this)','class'=>'table-form'));?>
	...
	<table >
		<tr>
			...
			<th width="20%"><?php echo __('publish').'/'.__('unpublish') ?></th>
		...
		<li id="listItem_<?=$owner['Owner']['id']?>" class="handle">
		<table width="100%">
		<tr>
			<td width="5%"><?php echo $this->Form->checkbox($owner['Owner']['id'],array('value'=>$owner['Owner']['id'])); ?></td>
			<td width="5%" ><?php echo $i++; ?></td>
			<td width="50%"><?php echo $owner['Owner']['name']; ?></td>
			<td width="10%"><?php echo $owner['Owner']['tax_code']; ?></td>
			<td width="20%">
				<?php $image = ($owner['Owner']['status']==1)?'admin/icon_success.png':'admin/icon_notification_error.png';?>
				<?=$this->Html->image($image);?>
			</td >
			<td width="20%">
				<ul class="actions">
					<li><?php echo $this->Html->link('edit', array('controller'=>'owners', 'action' => 'edit',$owner['Owner']['id']), array('escape' => false,'class'=>'edit','title'=>'Edit Item','rel'=>'tooltip'));?></li>
					<li>
						<?=$this->Html->link('view', array('controller'=>'owners', 'action' => 'view', $owner['Owner']['id']), array('escape' => false,'class'=>'view fancybox','title'=>'View Item','rel'=>'tooltip'))?>
					</li>
					<?php //if($parent_id==0){ ?>
					<li>
    ...
    $("#mylist").sortable({
		update : function () {
			 var order = $('#mylist').sortable('serialize');
				...
				$.get('<?=Configure::read('HTTP_PATH');?>/admin/owners/change_order/?'+order, function(data) {
				  //alert(data);
...
//	if(action=='Delete')
	...
	if(flag)
	{
		...
		if(validate())
			document.getElementById('OwnerDeleteForm').submit();
	}
}

function validate(){
		var ans="0";
		for(i=0; i<document.owner.elements.length; i++){
			if(document.owner.elements[i].type=="checkbox"){
				if(document.owner.elements[i].checked){
					ans="1";
					break;
				}
			}
		...
		if(ans=="0"){
			alert(" <?php echo __('Please').' '.__('select') .' '.__('owner').' '.__('to') ?> "+document.getElementById('action').value);
			return false;
		}else{
			var answer = confirm(' <?php echo __('are').' '.__('you') .' '.__('sure').' '.__('you').' '.__('want').' '.__('to')?> '+document.getElementById('action').value+' <?php echo __('owner') ?>');
			if(!answer)
				return false;
		}
		return true;
...
//alert(document.getElementsByTagName('checkbox').length);
	var fmobj=document.getElementById('OwnerDeleteForm');
	for (var i=0;i<fmobj.elements.length;i++) 
	{
		var e = fmobj.elements[i];
		if(e.type=='checkbox')
			fmobj.elements[i].checked=document.getElementById('OwnerCheck').checked;
	}
...
function change_url(){
	...
	document.location.href = '<?=Configure::read('HTTP_PATH');?>/admin/owners/index/'+search+'/'+limit;
