...
<div>
	<article>
		<header>
			<h2><?php echo __('language_manager') ?></h2>
			<div style="float:right;">
				...
				<?php echo $this->Html->link(__('new'), array('controller'=>'languages', 'action' => 'admin_add'), array('escape' => false,'class'=>'button'));?>
			</div>
		</header>
	</article>
	...
	<?php echo $this->Form->create('Language', array('name'=>'language','action' => 'delete/','id'=>'LanguageDeleteForm','onSubmit'=>'return validate(this)','class'=>'table-form'));?>
	...
	<table >
		<tr>
			...
			<th width="5%">SNo.</th>
			<th width="50%"><?php echo __('language') ?></th>
		...
		<li id="listItem_<?=$language['Language']['id']?>" class="handle">
		<table width="100%">
		<tr>
			<td width="5%"><?php echo $this->Form->checkbox($language['Language']['id'],array('value'=>$language['Language']['id'])); ?></td>
			<td width="5%" ><?php echo $i++; ?></td>
			<td width="50%"><?php echo $language['Language']['name']; ?></td>
			<td width="10%"><?php echo $language['Language']['code']; ?></td>
			<td width="20%">
				<?php $image = ($language['Language']['status']==1)?'admin/icon_success.png':'admin/icon_notification_error.png';?>
				<?=$this->Html->image($image);?>
			</td >
			<td width="20%">
				<ul class="actions">
					<li><?php echo $this->Html->link('edit', array('controller'=>'languages', 'action' => 'edit',$language['Language']['id']), array('escape' => false,'class'=>'edit','title'=>'Edit Item','rel'=>'tooltip'));?></li>
					<li>
						<?=$this->Html->link('view', array('controller'=>'languages', 'action' => 'view', $language['Language']['id']), array('escape' => false,'class'=>'view fancybox','title'=>'View Item','rel'=>'tooltip'))?>
					</li>
					<?php //if($parent_id==0){ ?>
					<li>
...
function validate(){
		var ans="0";
		for(i=0; i<document.language.elements.length; i++){
			if(document.language.elements[i].type=="checkbox"){
				if(document.language.elements[i].checked){
					ans="1";
					break;
				}
			}
		...
		if(ans=="0"){
			alert(" <?php echo __('Please').' '.__('select') .' '.__('language').' '.__('to') ?>  "+document.getElementById('action').value);
			return false;
		}else{
			var answer = confirm('<?php echo __('are').' '.__('you') .' '.__('sure').' '.__('you').' '.__('want').' '.__('to')?> '+document.getElementById('action').value+' <?php echo __('language') ?>');
			if(!answer)
				return false;
		}
		return true;
...
//alert(document.getElementsByTagName('checkbox').length);
	var fmobj=document.getElementById('LanguageDeleteForm');
...
function change_url(){
	...
	document.location.href = '<?=Configure::read('HTTP_PATH');?>/admin/languages/index/'+search+'/'+limit;
