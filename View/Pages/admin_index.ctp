...
<div>
  <article>
    <header>
      <h2><?php echo __('content_manager') ?><?php echo (!empty($parent_detail)) ? '[' . $parent_detail['Page']['pageTitle'] . ']' : ''; ?></h2>
      <div style="float:right;">
        ...
        <?php echo $this->Html->link(__('new'), array('controller' => 'pages', 'action' => 'add', $parent_id), array('escape' => false, 'class' => 'button')); ?>
      </div>
    </header>
  </article>
  ...
  <?php echo $this->Form->create('Page', array('name' => 'page', 'action' => 'delete/' . $parent_id, 'id' => 'PageDeleteForm', 'onSubmit' => 'return validate(this)', 'class' => 'table-form')); ?>
  <?php echo $this->Form->hidden('action', array('id' => 'action', 'value' => '')); ?>
  <?php echo $this->Form->hidden('parentId', array('id' => 'parentId', 'value' => $parent_id)); ?>
  <table >
    <tr>
      ...
      <th width="20%"><?=__('page')?> <?=__('title')?></th>
      
      <th width="10%"><?php echo __('language') ?></th>
      <th width="20%"><?php echo __('publish') ?>/<?php echo __('unpublish') ?></th>
      ...
      <td colspan="8">
        <ul id="mylist"  class="Main" >
          ...
          <?php foreach ($pages as $page) { ?>
            <li id="listItem_<?= $page['Page']['id'] ?>" class="handle">
              <table width="100%">
                <tr>
                  <td width="5%"><?php echo $this->Form->checkbox($page['Page']['id'], array('value' => $page['Page']['id'])); ?></td>
                  <td width="5%" ><?php echo $i++; ?></td>
                  <td width="20%"><?php echo $page['Page']['name']; ?></td>
                  <td width="20%"><?php echo $page['Page']['pageTitle']; ?></td>
                  <td width="10%"><?php echo $page['Page']['language_code']; ?></td>
                  <td width="20%">
                    <?php $image = ($page['Page']['status'] == 1) ? 'admin/icon_success.png' : 'admin/icon_notification_error.png'; ?>
                    <?= $this->Html->image($image); ?>
                  </td >
                  <td width="20%">
                    <ul class="actions">
                      <li><?php echo $this->Html->link('edit', array('controller' => 'pages', 'action' => 'edit', $parent_id, $page['Page']['id']), array('escape' => false, 'class' => 'edit', 'title' => 'Edit Item', 'rel' => 'tooltip')); ?></li>
                      <li>
                        <?= $this->Html->link('view', array('controller' => 'pages', 'action' => 'view', $page['Page']['id']), array('escape' => false, 'class' => 'view fancybox', 'title' => 'View Item', 'rel' => 'tooltip')) ?>
                      </li>
                      <?php if ($parent_id == 0) { ?>
                      <li>
                          <?= $this->Html->link('Manage Sub Content', array('controller' => 'pages', 'action' => 'index', $page['Page']['id']), array('escape' => false, 'class' => 'subcontent', 'title' => 'Manage Sub Content', 'rel' => 'tooltip')) ?>
                        </li>
                              ...
                              $(document).ready(function() {
                                ...
                                $("#mylist").sortable({
                                  update: function() {
                                    ...
                                    $.get('<?= Configure::read('HTTP_PATH'); ?>/admin/pages/change_order/?' + order, function(data) {
                                      //alert(data);
...
//	if(action=='Delete')
  ...
  function validate() {
    ...
    if (ans == "0") {
      alert("<?php echo __('Please') . ' ' . __('select') . ' ' . __('content') . ' ' . __('to') ?>" + document.getElementById('action').value);
      return false;
    } else {
      var answer = confirm('<?php echo __('are') . ' ' . __('you') . ' ' . __('sure') . ' ' . __('you') . ' ' . __('want') . ' ' . __('to') ?>' + document.getElementById('action').value + ' Page(s)');
      if (!answer)
        return false;
    }
    return true;
...
//alert(document.getElementsByTagName('checkbox').length);
  ...
  function change_url() {
    ...
    document.location.href = '<?= Configure::read('HTTP_PATH'); ?>/admin/pages/index/' + parent_id + '/' + search + '/' + limit;
